...
{
    private const string UserAgent = "SpotifyByYear/0.1 ( https://github.com/psandler/SpotifyByYear )";
    ...
    /// <summary>Recording search by title and artist (up to 100 results, since live/reissue recordings crowd the top).</summary>
    ...
    private async Task<JsonDocument?> GetJsonAsync(string relativeUrl, CancellationToken cancellationToken)
    {
        for (var attempt = 1; ; attempt++)
        {
            ...
            // One request at a time, spaced at least MinInterval apart.
            ...
            try
            {
                ...
                {
                    _nextRequestAt = DateTimeOffset.UtcNow + MinInterval;
                }
                ...
                if (response.StatusCode is HttpStatusCode.ServiceUnavailable or HttpStatusCode.TooManyRequests &&
                    attempt < MaxAttempts)
                {
                    var backoff = response.Headers.RetryAfter?.Delta ?? TimeSpan.FromSeconds(5 * attempt);
